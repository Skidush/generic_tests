...
import { Item } from "../helpers/test-helpers";
...
import { ItemList } from "../po/item-list.po";
...
export class GenericHelper {
  ...
  static getPluralName(itemName: string) {
    const rawData = fs.readFileSync(root + "/data/Generic/itemNames.td.json");
  ...
  static async createItem(itemName: string) {
    const formSchemaWithValues = Item.getFormSchemaWithValues(itemName);
    const rawItemValues = Item.getRawValues(formSchemaWithValues);
  ...
  static async editItem(itemName: string) {
    ...
    const tableConfig = Item.getItemConfig(itemName).tableConfig;

    let randomItem: any = await ReportingDB.getItemTableData(itemName, tableConfig);

    const randomIndex = Item.randomWholeNumber(0, randomItem.length - 1);
    
    randomItem = randomItem[randomIndex];

    await ElementIs.present(ItemList.getTableRows().get(randomIndex));
    const tableRows = await ItemList.getTableRows().get(randomIndex);
